...
/** Compact header showing which account (platform vs school) the payment targets. */
...
> = ({ bankDetails, primaryHeadingLabel }) => (
  <div className="mb-6 p-4 rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-gray-800 flex items-center gap-4 animate-fade-in">
    <div
      className={`size-12 rounded-xl flex items-center justify-center text-white shrink-0 shadow-lg ${bankDetails.isLopayEscrow ? "bg-primary" : "bg-success"}`}
    >
      <span className="material-symbols-outlined text-2xl">
        {bankDetails.isLopayEscrow ? "verified_user" : "account_balance"}
      </span>
    </div>
